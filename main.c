...
void split_cell(cell* cell_el,cell* cell_list){
	if(cell_el->energy<80){
		printf("The cell doesnt have enough energy to split up.\n");
		return ;
	}else{
		int x=cell_el->x;
		int y=cell_el->y;
		...
		do{
			...
			// we do this loop 120 times to be sure about considering all the possibilites
			counter++;
			if(counter==120)break;	
		}
		}while(map[y+y_temp][x+x_temp].full!=0);
...
void single_player_handler (block** map){
	...
	scanf("%d",&energy);
	cell* player_cell=create_list(cell_num,map,'X',energy);
	//the output of cell_action function is an int value and when user selects 5 it returns 0 value
	while(cell_action(choose_cell(player_cell),player_cell));
	//menu(map);
